...
export const useUserProfile = (userId?: string, options: UseUserProfileOptions = {}) => {
  ...
  const {
    ...
    isOffline,
    isFromCache,
  } = useData<UserProfile>(id ? `/users/${id}/profile` : null, {
  ...
  const {
    ...
    error: updateError,
  } = useMutation<UserProfile>(id ? `/users/${id}/profile` : null, {
  ...
  const updateStatus = useCallback(
    async (status: UserProfile['status']) => {
  ...
  const updatePrivacySettings = useCallback(
    async (privacy: Partial<UserPreferences['privacy']>) => {
      if (!id || !profile || !profile.preferences) return false;
  ...
  const updateAccessibilitySettings = useCallback(
    async (accessibility: Partial<UserPreferences['accessibility']>) => {
      if (!id || !profile || !profile.preferences) return false;
