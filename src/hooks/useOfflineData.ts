...
function useOfflineData<T extends { id: string }>(
  ...
  const fetchFromServer = useCallback(async (): Promise<T | null> => {
    ...
    try {
      ...
      if (serverData) {
        ...
        await indexedDBService.setCache(storeName, serverData, cacheTime);
  ...
  const getData = useCallback(async () => {
    ...
    try {
      ...
      const cachedData = await indexedDBService.getCache<T & { lastUpdated?: number }>(
        storeName,
        id,
      );
  ...
  const saveData = useCallback(
    async (newData: Partial<T>): Promise<T | null> => {
      try {
        ...
        await indexedDBService.setCache(storeName, updatedData, cacheTime);

        // If online and endpoint is provided, sync with server
        if (isOnline && apiEndpoint) {
          try {
            const url = isUpdate ? `${apiEndpoint}/${itemId}` : apiEndpoint;
            ...
            await indexedDBService.setCache(storeName, finalData, cacheTime);
    ...
    try {
      ...
      if (isOnline && apiEndpoint) {
        try {
          ...
          if (apiEndpoint) {
            await addToOfflineQueue(`${apiEndpoint}/${id}`, 'DELETE', null);
          }
        }
      } else if (apiEndpoint) {
        // If offline, add to sync queue
...
export function useOfflineCollection<T extends { id: string }>(
  ...
  const fetchFromServer = useCallback(async (): Promise<T[] | null> => {
    ...
    try {
      ...
      if (serverData && Array.isArray(serverData)) {
        ...
        await Promise.all(
          serverData.map(item => indexedDBService.setCache(storeName, item, cacheTime)),
        );
  ...
  const getCollectionData = useCallback(async () => {
    try {
      ...
      if (collectionMeta) {
        ...
        // Apply filter if provided
        let filteredItems = filterFn ? validItems.filter(filterFn) : validItems;
  ...
  const addItem = useCallback(
    async (newItem: Partial<T>): Promise<T | null> => {
      try {
        ...
        // Cache the new item
        await indexedDBService.setCache(storeName, item, cacheTime);
        ...
        if (isOnline && options.apiEndpoint) {
          try {
            ...
            if (optimisticUpdates) {
              setItems(prevItems => {
                const updatedItems = prevItems.map(i => (i.id === itemId ? serverData : i));
            ...
            if (options.apiEndpoint) {
              await addToOfflineQueue(options.apiEndpoint, 'POST', item);
            }
          }
  ...
  const updateItem = useCallback(
    async (itemId: string, updates: Partial<T>): Promise<T | null> => {
      try {
        setError(null);
        ...
        if (optimisticUpdates) {
          setItems(prevItems => {
            const updatedItems = prevItems.map(item => (item.id === itemId ? updatedItem : item));
        ...
        await indexedDBService.setCache(storeName, updatedItem, cacheTime);
        ...
        if (isOnline && options.apiEndpoint) {
          try {
            ...
            // Queue for sync when online
            if (options.apiEndpoint) {
              await addToOfflineQueue(`${options.apiEndpoint}/${itemId}`, 'PATCH', updates);
            }
          }
        } else if (options.apiEndpoint) {
          // If offline, add to sync queue
      ...
      try {
        ...
        if (isOnline && options.apiEndpoint) {
          try {
            ...
            // Queue delete operation for when online
            if (options.apiEndpoint) {
              await addToOfflineQueue(`${options.apiEndpoint}/${itemId}`, 'DELETE', null);
            }
          }
        } else if (options.apiEndpoint) {
          // If offline, add to sync queue
