...
export const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(subcommand =>
      subcommand.setName('setup').setDescription('Initialize or reinitialize the server structure'),
    )
    .addSubcommand(subcommand =>
      subcommand.setName('status').setDescription('Check server status and configuration'),
    )
    .addSubcommand(subcommand =>
      subcommand.setName('fix').setDescription('Fix missing channels and roles'),
    )
    .toJSON(),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    try {
      ...
      await interaction.editReply(
        '❌ An error occurred while executing the command. Please check the logs for details.',
      );
    }
  },
...
async function handleSetup(interaction: ChatInputCommandInteraction, guild: Guild) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0x00ff00)
      .setDescription(
        'The server has been successfully initialized with all required channels, roles, and configurations.',
      )
      .addFields(
...
async function handleFix(interaction: ChatInputCommandInteraction, guild: Guild) {
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setColor(0xffa500)
      .setDescription(
        'The server has been checked and any missing channels or roles have been recreated.',
      )
      .setTimestamp();
