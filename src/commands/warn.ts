...
export const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option =>
      option.setName('guidance').setDescription('Guidance for improvement').setRequired(true),
    )
    .toJSON(),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    try {
      ...
      await interaction.editReply(
        `✅ Warning issued to ${targetUser.tag} with wisdom and guidance.`,
      );
      ...
      await interaction.editReply(
        '❌ An error occurred while issuing the warning. Please check the logs for details.',
      );
    }
  },
