...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption(option =>
    option.setName('model')
      .setDescription('Specific model to use')
      .setRequired(false))
  ...
  try {
    ...
    if (availableProviders.length === 0) {
      await interaction.editReply({
        content: '❌ No AI providers are currently available. Please check your API keys and try again.',
        embeds: [createErrorEmbed('No AI Providers Available', 'All configured AI providers are unavailable or have invalid API keys.')]
      });
      return;
    }
    ...
    logger.info(`AI response generated successfully using ${response.model}, cost: $${response.cost.toFixed(4)}`);
    ...
    if (error instanceof AIProviderError) {
      switch (error.type) {
        case AIErrorType.AUTHENTICATION:
          ...
          errorEmbed = createErrorEmbed('Authentication Error', 'Invalid or missing API key for the selected AI provider.');
          break;
        case AIErrorType.RATE_LIMIT:
          errorMessage = '⏰ Rate limit exceeded. Please wait a moment and try again.';
          errorEmbed = createErrorEmbed('Rate Limit', 'Too many requests. Please wait before trying again.');
          break;
        ...
        case AIErrorType.MODEL_UNAVAILABLE:
          ...
          errorEmbed = createErrorEmbed('Model Unavailable', 'The selected AI model is not available at the moment.');
          break;
        case AIErrorType.INVALID_REQUEST:
          errorMessage = '📝 Invalid request parameters. Please check your input.';
          errorEmbed = createErrorEmbed('Invalid Request', 'The request parameters are invalid or malformed.');
          break;
        default:
...
function createResponseEmbed(response: any, params: AIRequestParams): EmbedBuilder {
  ...
  if (params.systemPrompt) {
    embed.addFields({ name: 'System Prompt', value: params.systemPrompt.substring(0, 100) + (params.systemPrompt.length > 100 ? '...' : ''), inline: false });
  }
...
export const metadata = {
  ...
  cooldown: 10,
  permissions: [],
  usage: '/ai prompt: "Your question here" [provider] [model] [max_tokens] [temperature] [system_prompt]',
  examples: [
