...
const EditProfileModal = ({ profile, onClose, onSave }) => {
  ...
  const removeGame = gameName => {
    handleChange(
      'games',
      formData.games.filter(g => g.name !== gameName),
    );
  };
          ...
          <Box>
            ...
            <Box mb={2}>
              <Grid container spacing={1}>
                {formData.languages.map(lang => (
                  <Grid item key={lang}>
                    <Chip label={lang} onDelete={() => removeLanguage(lang)} />
                  </Grid>
                ))}
              </Grid>
            </Box>
            <Box display='flex' gap={1}>
              <TextField
                ...
                size='small'
              />
              <Button variant='outlined' onClick={addLanguage} disabled={!newLanguage}>
                Add
              </Button>
            </Box>
          </Box>
              ...
              <Grid item xs={12} sm={4}>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    onChange={e =>
                      handleNestedChange('preferences', 'communication', e.target.value)
                    }
                    renderValue={selected => (
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                        {selected.map(value => (
