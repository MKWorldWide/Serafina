...
export const useMessaging = (): UseMessagingReturn => {
  ...
  const currentPagination = activeConversationId
    ? messagesPagination[activeConversationId]
    : undefined;
  ...
  useEffect(() => {
    if (
      activeConversationId &&
      (!allMessages[activeConversationId] || allMessages[activeConversationId].length === 0)
    ) {
